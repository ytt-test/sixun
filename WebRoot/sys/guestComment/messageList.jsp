...
<body>
	...
	<div id="QueryArea">
		<form action="${path}/servlet/message" method="post">
			<input type="hidden" name="method" value="list"> <input
				type="text" name="message" placeholder="请输入需要查询的留言内容"> <input
	...
	<div id="MainArea">
			...
			<tbody id="TableData">
				<c:choose>
					<c:when test="${not empty requestScope.pb.datas}">
						<c:forEach items="${requestScope.pb.datas }" var="msg"
							varStatus="vs">
							<tr class="TableDetail1">
								...
								<td>${msg.message}&nbsp;</td>
								<td><a
									href="${path}/servlet/message?method=findById&id=${msg.id}"
									class="FunctionButton">更新</a> <a
									href="${path}/servlet/message?method=delete&id=${msg.id}"
		...
		<div id="TableTail" align="center">
			...
			<a href="${path }/servlet/message?method=list">首页</a> <a
				href="${path }/servlet/message?method=list&pageNow=${requestScope.pb.pageNow-1}">上一页</a>
			<a
				href="${path }/servlet/message?method=list&pageNow=${requestScope.pb.pageNow+1}">下一页</a>
			<a
				href="${path }/servlet/message?method=list&pageNow=${requestScope.pb.totalPages}">末页</a>
